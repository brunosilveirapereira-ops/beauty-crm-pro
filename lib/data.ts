...
import type { Appointment, BeforeAfterHistory, Customer, DashboardStats, ProductHistory, Professional, ServiceHistory, VisitHistory } from "./types";
...
export async function getProductHistory(customerId: string): Promise<ProductHistory[]> {
  ...
  const salonId = await getCurrentSalonId();
  if (!salonId) {
    console.info("[Beauty CRM Pro] Sem salao resolvido para o utilizador atual: a devolver historico de produtos vazio.");
    return [];
  }

  console.info("[Beauty CRM Pro] Supabase conectado: lendo produtos utilizados de public.product_history.", { customerId, salonId });
